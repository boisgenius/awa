# Claw Academy 功能实现计划

> 基于 `功能实现状态.md` 制定的详细实现计划
> 创建日期: 2026-02-06

## 目录

1. [项目概述](#项目概述)
2. [Phase 1: 核心交互功能](#phase-1-核心交互功能高优先级)
3. [Phase 2: 页面扩展](#phase-2-页面扩展中优先级)
4. [Phase 3: 用户功能](#phase-3-用户功能低优先级)
5. [技术规范](#技术规范)
6. [测试计划](#测试计划)

---

## 项目概述

### 当前状态
- UI 组件已基于原型实现
- API 路由已搭建（使用 Mock 数据）
- 数据类型已定义 (`src/lib/skills/types.ts`)

### 技术栈
- **前端**: Next.js 14 (App Router), React 18, TypeScript
- **样式**: CSS (globals.css)
- **后端**: Next.js API Routes
- **数据库**: Supabase (待集成)
- **区块链**: Solana (待集成)

---

## Phase 1: 核心交互功能（高优先级）

预计工时: 2-3 天

### 1.1 搜索功能

**目标**: 实现技能关键词搜索

**涉及文件**:
- `src/app/(dashboard)/marketplace/page.tsx`
- `src/app/(dashboard)/layout.tsx`
- `src/hooks/useSearch.ts` (新建)

**实现步骤**:

```typescript
// 1. 创建搜索 Hook
// src/hooks/useSearch.ts
import { useState, useCallback, useMemo } from 'react';
import { useDebouncedCallback } from 'use-debounce';

export function useSearch<T>(
  items: T[],
  searchKeys: (keyof T)[],
  debounceMs = 300
) {
  const [query, setQuery] = useState('');

  const debouncedSetQuery = useDebouncedCallback(setQuery, debounceMs);

  const filteredItems = useMemo(() => {
    if (!query.trim()) return items;
    const lowerQuery = query.toLowerCase();
    return items.filter(item =>
      searchKeys.some(key => {
        const value = item[key];
        return typeof value === 'string' &&
               value.toLowerCase().includes(lowerQuery);
      })
    );
  }, [items, query, searchKeys]);

  return { query, setQuery: debouncedSetQuery, filteredItems };
}

// 2. 在 marketplace/page.tsx 中使用
const { query, setQuery, filteredItems } = useSearch(
  skills,
  ['name', 'description', 'author']
);
```

**验收标准**:
- [ ] 输入框支持实时搜索
- [ ] 搜索支持名称、描述、作者
- [ ] 300ms 防抖避免频繁请求
- [ ] 空搜索显示全部结果

---

### 1.2 ⌘K 快捷键

**目标**: 按 ⌘K (Mac) 或 Ctrl+K (Windows) 聚焦搜索框

**涉及文件**:
- `src/app/(dashboard)/layout.tsx`
- `src/hooks/useKeyboardShortcut.ts` (新建)

**实现步骤**:

```typescript
// 1. 创建快捷键 Hook
// src/hooks/useKeyboardShortcut.ts
import { useEffect, useCallback } from 'react';

export function useKeyboardShortcut(
  key: string,
  callback: () => void,
  options: { ctrlKey?: boolean; metaKey?: boolean } = {}
) {
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    const isMac = navigator.platform.toUpperCase().indexOf('MAC') >= 0;
    const modifierPressed = isMac ? event.metaKey : event.ctrlKey;

    if (event.key.toLowerCase() === key.toLowerCase() && modifierPressed) {
      event.preventDefault();
      callback();
    }
  }, [key, callback]);

  useEffect(() => {
    document.addEventListener('keydown', handleKeyDown);
    return () => document.removeEventListener('keydown', handleKeyDown);
  }, [handleKeyDown]);
}

// 2. 在 layout.tsx 中使用
const searchInputRef = useRef<HTMLInputElement>(null);

useKeyboardShortcut('k', () => {
  searchInputRef.current?.focus();
});

// 3. 绑定 ref 到搜索框
<input ref={searchInputRef} type="text" placeholder="Search skills..." />
```

**验收标准**:
- [ ] Mac 上 ⌘K 聚焦搜索框
- [ ] Windows 上 Ctrl+K 聚焦搜索框
- [ ] 聚焦时输入框有视觉反馈

---

### 1.3 分类筛选

**目标**: 点击分类 Pill 过滤技能列表

**涉及文件**:
- `src/app/(dashboard)/marketplace/page.tsx`

**实现步骤**:

```typescript
// 1. 修改状态管理
const [activeCategory, setActiveCategory] = useState<string>('all');

// 2. 过滤逻辑
const filteredByCategory = useMemo(() => {
  if (activeCategory === 'all') return skills;
  return skills.filter(skill =>
    skill.category.toLowerCase() === activeCategory
  );
}, [skills, activeCategory]);

// 3. 更新 API 调用（集成后）
useEffect(() => {
  const params = new URLSearchParams();
  if (activeCategory !== 'all') {
    params.set('category', activeCategory);
  }
  // fetch(`/api/skills?${params}`)
}, [activeCategory]);
```

**验收标准**:
- [ ] 点击分类切换筛选
- [ ] 显示当前激活分类
- [ ] "All" 显示全部技能
- [ ] 分类计数正确更新

---

### 1.4 排序功能

**目标**: 下拉选择排序方式

**涉及文件**:
- `src/app/(dashboard)/marketplace/page.tsx`

**实现步骤**:

```typescript
// 1. 定义排序类型
type SortOption = 'trending' | 'newest' | 'downloads' | 'rating';

// 2. 添加状态
const [sortBy, setSortBy] = useState<SortOption>('trending');

// 3. 排序逻辑
const sortedSkills = useMemo(() => {
  const sorted = [...filteredSkills];
  switch (sortBy) {
    case 'newest':
      return sorted.sort((a, b) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );
    case 'downloads':
      return sorted.sort((a, b) => b.downloads - a.downloads);
    case 'rating':
      return sorted.sort((a, b) => b.rating - a.rating);
    case 'trending':
    default:
      return sorted.sort((a, b) =>
        (b.downloads * b.rating) - (a.downloads * a.rating)
      );
  }
}, [filteredSkills, sortBy]);

// 4. 绑定下拉框
<select
  className="sort-select"
  value={sortBy}
  onChange={(e) => setSortBy(e.target.value as SortOption)}
>
  <option value="trending">Sort: Trending</option>
  <option value="newest">Sort: Newest</option>
  <option value="downloads">Sort: Most Downloads</option>
  <option value="rating">Sort: Highest Rated</option>
</select>
```

**验收标准**:
- [ ] 下拉选择排序方式
- [ ] 实时更新排序结果
- [ ] 默认按 Trending 排序

---

### 1.5 API 数据集成

**目标**: 替换 Mock 数据为 API 调用

**涉及文件**:
- `src/app/(dashboard)/marketplace/page.tsx`
- `src/app/(dashboard)/leaderboard/page.tsx`
- `src/hooks/useSkills.ts` (新建)

**实现步骤**:

```typescript
// 1. 创建数据获取 Hook
// src/hooks/useSkills.ts
import useSWR from 'swr';
import type { Skill, SkillQuery, PaginatedResult } from '@/lib/skills';

const fetcher = (url: string) => fetch(url).then(res => res.json());

export function useSkills(query: SkillQuery = {}) {
  const params = new URLSearchParams();
  if (query.category) params.set('category', query.category);
  if (query.search) params.set('search', query.search);
  if (query.sort) params.set('sort', query.sort);
  if (query.page) params.set('page', String(query.page));
  if (query.limit) params.set('limit', String(query.limit));

  const { data, error, isLoading, mutate } = useSWR<PaginatedResult<Skill>>(
    `/api/skills?${params}`,
    fetcher
  );

  return {
    skills: data?.data ?? [],
    total: data?.total ?? 0,
    hasMore: data?.hasMore ?? false,
    isLoading,
    isError: error,
    mutate,
  };
}

// 2. 安装 SWR
// npm install swr

// 3. 在 marketplace 中使用
const { skills, isLoading, hasMore } = useSkills({
  category: activeCategory === 'all' ? undefined : activeCategory,
  search: searchQuery,
  sort: sortBy,
  page: 1,
  limit: 20,
});
```

**验收标准**:
- [ ] 页面加载时从 API 获取数据
- [ ] 支持 loading 状态显示
- [ ] 支持错误处理
- [ ] 支持数据缓存和重新验证

---

## Phase 2: 页面扩展（中优先级）

预计工时: 3-4 天

### 2.1 /trending 页面

**路径**: `src/app/(dashboard)/trending/page.tsx`

**功能**:
- 显示热门技能列表
- 基于 `downloads * rating` 排序
- 复用 Marketplace 的 Skills Grid

```typescript
// src/app/(dashboard)/trending/page.tsx
'use client';

import { useSkills } from '@/hooks/useSkills';
import SkillsGrid from '@/components/SkillsGrid';

export default function TrendingPage() {
  const { skills, isLoading } = useSkills({ sort: 'trending', limit: 50 });

  return (
    <section className="page-section">
      <h1 className="page-title">🔥 Trending Skills</h1>
      <p className="page-desc">Most popular skills this week</p>
      <SkillsGrid skills={skills} isLoading={isLoading} />
    </section>
  );
}
```

---

### 2.2 /top-rated 页面

**路径**: `src/app/(dashboard)/top-rated/page.tsx`

**功能**:
- 显示高评分技能
- 基于 `rating` 排序，筛选 rating >= 4.5

---

### 2.3 /category/[slug] 页面

**路径**: `src/app/(dashboard)/category/[slug]/page.tsx`

**功能**:
- 动态分类页面
- URL: `/category/research`, `/category/finance`, etc.
- 显示该分类下所有技能

```typescript
// src/app/(dashboard)/category/[slug]/page.tsx
interface PageProps {
  params: { slug: string };
}

const categoryMap: Record<string, { emoji: string; name: string }> = {
  research: { emoji: '🔬', name: 'Research' },
  finance: { emoji: '📈', name: 'Finance' },
  coding: { emoji: '💻', name: 'Coding' },
  security: { emoji: '🛡️', name: 'Security' },
  creative: { emoji: '🎨', name: 'Creative' },
};

export default function CategoryPage({ params }: PageProps) {
  const category = categoryMap[params.slug];
  const { skills, isLoading } = useSkills({ category: params.slug });

  if (!category) return notFound();

  return (
    <section className="page-section">
      <h1 className="page-title">{category.emoji} {category.name}</h1>
      <SkillsGrid skills={skills} isLoading={isLoading} />
    </section>
  );
}
```

---

### 2.4 /skills/[id] 页面重写

**路径**: `src/app/(dashboard)/skills/[id]/page.tsx`

**功能**:
- 技能详情页
- 显示完整描述、特性、评分、作者信息
- 下载/购买按钮
- 版本历史（可选）

**UI 结构**:
```
┌─────────────────────────────────────────┐
│ [Icon]  Skill Name          [★ Rating] │
│         by Author           [Verified] │
├─────────────────────────────────────────┤
│ Description...                          │
├─────────────────────────────────────────┤
│ Features: [Tag] [Tag] [Tag]             │
├─────────────────────────────────────────┤
│ [Download Button]    Downloads: 1,234   │
└─────────────────────────────────────────┘
```

---

### 2.5 Load More 分页

**目标**: 滚动或点击加载更多技能

**实现方式**:
- 点击 "Load More" 按钮
- 或使用 Intersection Observer 无限滚动

```typescript
// 状态管理
const [page, setPage] = useState(1);
const [allSkills, setAllSkills] = useState<Skill[]>([]);

const { skills, hasMore, isLoading } = useSkills({
  ...query,
  page,
  limit: 20,
});

// 累积数据
useEffect(() => {
  if (skills.length > 0) {
    setAllSkills(prev => page === 1 ? skills : [...prev, ...skills]);
  }
}, [skills, page]);

// Load More 按钮
<button
  onClick={() => setPage(p => p + 1)}
  disabled={!hasMore || isLoading}
>
  {isLoading ? 'Loading...' : 'Load More Skills ↓'}
</button>
```

---

### 2.6 收藏功能

**目标**: 保存/取消收藏技能

**涉及文件**:
- `src/app/(dashboard)/marketplace/page.tsx`
- `src/hooks/useFavorites.ts` (新建)
- `src/app/api/favorites/route.ts` (新建)

**数据模型**:
```typescript
interface Favorite {
  id: string;
  agentId: string;
  skillId: string;
  createdAt: Date;
}
```

**实现要点**:
- 使用 localStorage 临时存储（无认证时）
- API 认证后同步到服务器
- 乐观更新 UI

---

## Phase 3: 用户功能（低优先级）

预计工时: 4-5 天

### 3.1 /saved 页面

**路径**: `src/app/(dashboard)/saved/page.tsx`

**功能**:
- 显示已收藏的技能列表
- 支持取消收藏

---

### 3.2 /purchased 页面

**路径**: `src/app/(dashboard)/purchased/page.tsx`

**功能**:
- 显示已购买的技能
- 下载/访问入口
- 购买历史记录

**依赖**:
- Agent 认证系统
- 购买记录 API

---

### 3.3 /settings 页面

**路径**: `src/app/(dashboard)/settings/page.tsx`

**功能**:
- Agent 配置管理
- 通知设置
- API Key 管理

---

### 3.4 /developers 页面

**路径**: `src/app/(dashboard)/developers/page.tsx`

**功能**:
- 开发者平台介绍
- SDK 文档入口
- API 文档链接
- 创建技能入口

---

### 3.5 Sidebar 折叠

**目标**: 首页隐藏侧边栏，其他页面显示

**实现**:
```typescript
// layout.tsx
const pathname = usePathname();
const isHomePage = pathname === '/';

<aside className={`sidebar ${isHomePage ? 'collapsed' : ''}`}>
```

**CSS**:
```css
.sidebar.collapsed {
  width: 0;
  padding: 0;
  overflow: hidden;
}

.main {
  margin-left: var(--sidebar-width);
  transition: margin-left 0.3s ease;
}

.sidebar.collapsed + .main {
  margin-left: 0;
}
```

---

### 3.6 响应式侧边栏

**目标**: 移动端抽屉模式

**实现**:
- 添加汉堡菜单按钮
- 侧边栏改为 position: fixed
- 点击遮罩层关闭

```css
@media (max-width: 768px) {
  .sidebar {
    position: fixed;
    left: -280px;
    z-index: 1000;
    transition: left 0.3s ease;
  }

  .sidebar.open {
    left: 0;
  }

  .sidebar-overlay {
    display: none;
    position: fixed;
    inset: 0;
    background: rgba(0, 0, 0, 0.5);
    z-index: 999;
  }

  .sidebar.open + .sidebar-overlay {
    display: block;
  }
}
```

---

## 技术规范

### 目录结构（最终）

```
src/
├── app/
│   ├── (dashboard)/
│   │   ├── layout.tsx
│   │   ├── page.tsx                 # 首页
│   │   ├── marketplace/page.tsx     # 市场
│   │   ├── leaderboard/page.tsx     # 排行榜
│   │   ├── trending/page.tsx        # 热门 (新)
│   │   ├── top-rated/page.tsx       # 高评分 (新)
│   │   ├── category/[slug]/page.tsx # 分类 (新)
│   │   ├── skills/[id]/page.tsx     # 技能详情
│   │   ├── saved/page.tsx           # 收藏 (新)
│   │   ├── purchased/page.tsx       # 已购买 (新)
│   │   ├── settings/page.tsx        # 设置 (新)
│   │   └── developers/page.tsx      # 开发者 (新)
│   └── api/
│       ├── skills/
│       ├── agents/
│       ├── favorites/               # 收藏 API (新)
│       └── leaderboard/
├── components/
│   ├── SkillCard.tsx                # 技能卡片 (提取)
│   ├── SkillsGrid.tsx               # 技能网格 (提取)
│   ├── SearchBox.tsx                # 搜索框 (提取)
│   └── Sidebar.tsx                  # 侧边栏 (提取)
├── hooks/
│   ├── useSearch.ts                 # 搜索 Hook (新)
│   ├── useKeyboardShortcut.ts       # 快捷键 Hook (新)
│   ├── useSkills.ts                 # 技能数据 Hook (新)
│   └── useFavorites.ts              # 收藏 Hook (新)
└── lib/
    ├── skills/
    ├── auth/
    └── payment/
```

### 依赖安装

```bash
# Phase 1 需要
npm install swr use-debounce

# Phase 2-3 可选
npm install @tanstack/react-query  # 替代 SWR
npm install framer-motion          # 动画效果
```

### API 规范

| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/skills` | GET | 获取技能列表 |
| `/api/skills/[id]` | GET | 获取技能详情 |
| `/api/skills/trending` | GET | 热门技能 |
| `/api/favorites` | GET/POST/DELETE | 收藏管理 |
| `/api/leaderboard` | GET | 排行榜数据 |

---

## 测试计划

### 单元测试

```typescript
// src/hooks/useSearch.test.ts
describe('useSearch', () => {
  it('should filter items by search query', () => {
    // ...
  });

  it('should debounce search input', () => {
    // ...
  });
});
```

### 集成测试

- [ ] 搜索功能端到端测试
- [ ] 分类筛选测试
- [ ] API 数据加载测试
- [ ] 分页功能测试

### 手动测试清单

- [ ] 所有页面在 Chrome/Safari/Firefox 正常显示
- [ ] 移动端响应式布局正常
- [ ] 键盘导航可用
- [ ] 加载状态正确显示
- [ ] 错误状态正确处理

---

## 实现顺序建议

```
Week 1:
├── Day 1-2: Phase 1.1-1.3 (搜索、快捷键、分类筛选)
├── Day 3: Phase 1.4-1.5 (排序、API 集成)

Week 2:
├── Day 1: Phase 2.1-2.3 (trending、top-rated、category 页面)
├── Day 2: Phase 2.4 (技能详情页重写)
├── Day 3: Phase 2.5-2.6 (分页、收藏)

Week 3:
├── Day 1-2: Phase 3.1-3.4 (用户页面)
├── Day 3: Phase 3.5-3.6 (响应式优化)
├── Day 4-5: 测试和修复
```

---

## 待确认事项

- [ ] 技能价格计价方式（参考 `docs/未来功能规划.md`）
- [ ] 是否需要 Agent 状态显示
- [ ] 收藏功能是否需要登录
- [ ] 是否需要 SSR/SSG 优化

---

*文档版本: v1.0*
*最后更新: 2026-02-06*
