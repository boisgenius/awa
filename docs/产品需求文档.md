...
**说明**: 无需用户手动连接钱包，Agent 使用绑定的钱包自动支付。
...
> **重要**: 所有 UI 开发必须参考原型文件 `prototype/ui/claw-academy.html`

该原型文件包含完整的视觉设计实现，包括：
- 完整的 CSS 变量定义（颜色、间距、圆角、动画等）
- 所有核心组件的 HTML/CSS 实现
- 响应式断点处理
- 交互效果和动画
...
对于原型中未包含的组件（如消息提醒、弹窗、Toast、Tooltip 等），可以：

1. **自行设计** - 遵循原型的设计风格，使用相同的：
   - CSS 变量（颜色、间距、圆角）
   - 字体家族（Inter、JetBrains Mono）
   - 动画时长和缓动函数
   - 边框和阴影样式

2. **使用第三方组件** - 推荐以下库（需自定义样式匹配主题）：
   - [Radix UI](https://radix-ui.com) - 无样式组件库
   - [Headless UI](https://headlessui.com) - Tailwind 官方组件
...
| 组件类型 | 设计要点 |
|----------|----------|
| Dialog/Modal | 背景 `var(--bg-card)`，边框 `var(--border-color)`，圆角 `var(--radius-lg)` |
| Toast/Notification | 使用相应状态颜色（success: `--accent-secondary`, error: `--accent-danger`） |
| Tooltip | 背景 `var(--bg-tertiary)`，字体 `var(--font-mono)`，圆角 `var(--radius-sm)` |
| Dropdown/Menu | 参考 `.sort-select` 样式 |
| Loading/Spinner | 使用 `var(--crimson)` 作为主色 |
| Progress | 使用 `var(--crimson)` 渐变背景 |
